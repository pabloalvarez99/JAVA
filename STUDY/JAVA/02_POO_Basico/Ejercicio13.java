...
class Libro {
    ...
    public boolean esLargo() {if (paginas > 300) {return true;} else return false;}
    public String mostrarInfo() {
        return titulo + " (" + año + ") - " + paginas + " páginas"
...
class Biblioteca {
    ...
    public void buscarPorAutor(String nombreAutor) {
        for (int i = 0; i < cantidadLibros; i++) {
            if (libros[i].getAutor().getNombre() == nombreAutor) {
                libros[i].mostrarInfo();
            }
        }
    }
    public int contarLibrosLargos() {
        int cont = 0;
        for (int i = 0; i < cantidadLibros; i++) {
            if (libros[i].getPaginas() > 300) {
                cont++;
            }
        }
        return cont;
